function [runoff,melt,q,qm,SW_flux] = runoff_prod(SW_down_out, LW_down_out, air_T_out, rh_out, hum_out, pressure_out, wind_speed_out,...
    ...
    refresh_albedo, tau_snow_cold, tau_snow_melt, ice_albedo, Io_ice, Io_water, Io_slush, ro_snow_initial,...
...
for tt = 1:t_num-1
    ...
    if model_stage == 1
        ...
        if enthalpy(grid_num(tt)+1,tt)<0 || grid_profile(grid_num(tt)+1,tt)<0.01
            ...
            grid_profile(grid_num(tt)+1,tt) = grid_profile(grid_num(tt)+1,tt)+liquid1(tt);% 无径流
