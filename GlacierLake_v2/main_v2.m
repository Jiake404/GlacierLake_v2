...
lake_in_ind = 1;                                %set as 1 until needs to be used
lake_in_z = zeros(lake_grid_num, t_num);        %(m) depth of each grid cell of lake. 'in' is short for insert to allow for seperate lake_T and lake_ind
lake_in_T = zeros(lake_grid_num, t_num);        %(k) temperature of each grid cell of lake
lake_in_e = zeros(lake_grid_num, t_num);        %(J) enthalpy of each grid cell of lake
lake_in_tau = ones(lake_grid_num, t_num)*tau_water; %tau values for lake insert
lake_in_SW_prop = zeros(lake_grid_num, t_num);  %SW propagation for lake insert
lake_in_lambda = zeros(lake_grid_num, t_num);
...
% save('predata_ponting.mat')
% load('predata_ponting.mat')
% albedo_num = 1;
%% MAIN LOOP
...
for tt = 1:t_num - 1
    ...
    elseif model_stage == 3 && track_a(1,tt) == 3 %main stage 3 section (not coming from another stage in same time step)
       ...
       if tt >= hydrograph_start
        ...
        [lake_albedo(tt)] = cal_lakealbedo(albedo_num,lake_depth(tt)*1.1);
        %albedo multiplier 
...
%% 
bm = sum(lambda(max(lake_insert_ind)+1:end,:).*grid_profile(max(lake_insert_ind)+1:end,:));%cumulative bottom ablation
